...
允许入口点使用的块加载类型列表。将被 webpack 自动填充。只有当使用一个函数作为入口选项并从那里返回 chunkLoading 配置项时才需要。
...
W> `output.module` 为实验特性，仅在 [`experiments.outputModule`](/configuration/experiments/#experiments) 设置为 `true` 的时候启用。
