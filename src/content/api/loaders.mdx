...
可以使用 [`experiments.executeModule`](/configuration/experiments/#experimentsexecutemodule) 启用。

- `request`: 加载模块的请求字符串
- `options`:
  - `layer`: 指定该模块放置/编译的层
  - `publicPath`: 用于构建模块的公共路径
- `callback`: 返回 exports 的可选函数。如果没有提供 callback 函数的话，`importModule` 将会返回一个 Promise。
